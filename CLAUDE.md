...
- **패키지 설치 우선순위**: nix 패키지(`home/core.nix`)를 우선 사용한다. nixpkgs에 해당 패키지가 있으면 반드시 nix로 설치한다. Brew cask(`modules/apps.nix`)는 nix에 패키지가 없거나 macOS 네이티브 앱(.app)이 필요한 경우 등 최후의 수단으로만 사용한다.
